...
userRouter.delete("/delete-profile-picture", verifyToken, async (req, res) => {
...
userRouter.get("/profile/:userId", verifyToken, async (req, res) => {
  const result = await getOtherUserData(req.params.userId);
  successResponse({ res, data: result });
});

userRouter.get("/profileVisitor/:userId", verifyToken, async (req, res) => {
